...
var _default = {
  ...
  canvas: {
    ...
    // 背景色 支持hex、rgba
    aspectRatio: [16, 9],
    // 宽高比 todo 此配置意义不大，在引用plugin的地方控制app dom的宽高即可，一般情况下宽度100%。备注： 和 plug-in 的需求冲突了。
    zoomRange: [1, 10],
    // 允许缩放的范围 todo max支持配置，min是根据算法动态计算的。 这个和 全部显示的 需求冲突。备注： 目前第一个参数无效
    // 2d 背景图
    backgroud_global_img: "/images/bg/d29145040c47045cd0d0aa438354e2f.jpg",
    // 背景图地址
    backgroud_global_l2_img: null,
    //"/images/bg/浅色平面光效.png", // 背景图地址
    // 3d 背景图
    backgroud_local_img: "/images/bg/d29145040c47045cd0d0aa438354e2f.jpg",
    // 背景图地址
    backgroud_local_l2_img: null,
    //"/images/bg/浅色2.5D光效.png", // 背景图地址
    animateOpenSpeed: 0,
    // 棋盘展开速度
    DEFAULT_NODE_SIZE: 80 * Math.sin(30),
    // 默认节点半径
    zoom: 1,
    // 初始缩放程度 todo 底层不支持设置zoom数值。 备注：这个和 全部显示的 需求冲突。
    position: [0, 0] // 初始位置 todo 底层没有开放setCenter方法。 备注：需要询问，这个看上去有点问题。

  },
  header: {
    bgImage: "./images/light/common/header-bg.png",
    // 背景图
    height: "60px",
    // 高度
    bgColor: "#032840",
  ...
  d2View: {
    ...
    selected: {
      borderWidth: 3,
      // 线粗 todo 动态改变selectedBorderWidth不生效，相关代码：_higlightNode、_reloadConfig 已修，改为监听 isMouseOver
      borderColor: "0,0,0",
      // 颜色 todo 动态改变selectedBorderColor不生效，相关代码：_higlightNode、_reloadConfig 已修，改为监听 isMouseOver
      borderAlpha: 0.3,
      // 透明度 todo 动态改变selectedBorderAlpha不生效，相关代码：_higlightNode、_reloadConfig 已修，改为监听 isMouseOver
      isBgColor: false,
      // 是否填色
      bgColor: "255,255,255",
      // 填色的颜色
    ...
    selectedLink: {
      width: 2,
      // 线粗 todo 底层selected.lineWidth不生效 已修
      color: "1,255,185",
      // 颜色
      alpha: 0.3,
      // 透明度
